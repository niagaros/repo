# Getting started with Niagaros
Niagaros is a React project with several dependencies. Follow these steps to set up and run the project on your local machine. 
...
Open your terminal and run the following command to clone the develop branch of the Niagaros repository:

`git clone https://github.com/niagaros/repo.git`

## Pre Amplify Setup
Before you start the backend setup make sure you `delete` the `amplify` map in the Niagaros folder inside your code editor (VsCode/Intellij). After you have deleted this folder enter the web folder (through your code editor terminal): `cd web`. Inside your web folder there is also an amplify map you can `delete` this one too (just to be sure). 
...
>**Hint**: Before running the upcoming command, ask for the login credentials to the Niagaros AWS account.

Ask for the login details to the Niagaros AWS account. Once you have the credentials, install the Amplify backend configuration using the following command: 
