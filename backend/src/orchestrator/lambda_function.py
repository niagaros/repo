...
def lambda_handler(event, context):
    ...
    try:
        ...
        # A scan can be scoped to one account ({"cloud_account_id": ...}) so that one customer's "Scan now"
        # never re-scans every other customer's AWS account.
        only = (event or {}).get("cloud_account_id")
        if only:
