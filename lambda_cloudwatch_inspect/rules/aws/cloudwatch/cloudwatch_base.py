...
class CloudWatchBaseCheck:
    ...
    Subclasses call self.evaluate(snapshot) which returns a CheckResult.
    """
    ...
    def _get_active_trail(self, snapshot: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Return the first trail that:
        - has logging enabled
        - is logging management events
        """
    ...
    def _find_metric_filter(
        ...
        """
        Find a metric filter on the given log group whose filterPattern
        matches (or contains) the required CIS filter pattern.

        Matching is normalised: whitespace collapsed, case-insensitive.
        """
    ...
    def _pattern_matches(self, candidate: str) -> bool:
        """
        Normalise both patterns (collapse whitespace, lowercase) and check
        whether the candidate contains the required pattern as a substring.
        """
    ...
    def _extract_metric_name(metric_filter: Dict[str, Any]) -> Optional[str]:
        """
        Extract the metric name from the first MetricTransformation.
        """
    ...
    def _find_alarm_for_metric(
        ...
        """
        Find a CloudWatch Alarm whose MetricName matches the given metric name.
        """
        if not metric_name:
            return None
    ...
    def _find_sns_action(alarm: Dict[str, Any]) -> Optional[str]:
        """
        Return the first SNS topic ARN found in AlarmActions, OKActions,
    ...
    def _has_active_subscription(
        ...
        """
        Return True if the SNS topic has at least one subscription that is
        not in PendingConfirmation state.
        """
