...
pytestmark = [pytest.mark.live, pytest.mark.flow("E2E-API-001"), pytest.mark.severity("P2")]
...
def test_invalid_account_id_is_rejected_cleanly_without_leaking_sql(anon, path):
    status, body, _ = anon.get(path, params={"cloud_account_id": "null"})
    text = str(body)
...
def test_invalid_account_id_degrades_to_an_empty_result(anon, path):
    status, _, _ = anon.get(path, params={"cloud_account_id": "null"})
    assert status == 200


def test_missing_account_id_is_a_400_where_required(anon):
    status, body, _ = anon.get("notifications")
...
def test_sql_injection_attempt_in_account_id_does_not_execute(anon):
    status, body, _ = anon.get("tprm", params={"cloud_account_id": "x'; DROP TABLE vendors;--"})
